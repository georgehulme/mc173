...
use mc173::block;
use mc173::entity::{self as e, BaseKind, Entity, EntityKind, LivingKind, ProjectileKind};
use mc173::geom::Face;
...
impl EntityTracker {
    ...
    pub fn set_vel(&mut self, vel: DVec3) {
        ...
        let scaled = vel
            .clamp(DVec3::splat(-3.9), DVec3::splat(3.9))
            .mul(8000.0)
            .as_ivec3();
    ...
    pub fn update_players(&mut self, players: &[ServerPlayer]) {
        ...
        let send_look = self.look.0.abs_diff(self.sent_look.0) >= 8
            || self.look.1.abs_diff(self.sent_look.1) >= 8;
        ...
        if let (false, Some(dx), Some(dy), Some(dz)) = (forced_position, dx, dy, dz) {
            ...
            if send_pos && send_look {
                move_packet = Some(OutPacket::EntityMoveAndLook(
                    proto::EntityMoveAndLookPacket {
                        entity_id: self.id,
                        dx,
                        dy,
                        dz,
            ...
            move_packet = Some(OutPacket::EntityPositionAndLook(
                proto::EntityPositionAndLookPacket {
                    entity_id: self.id,
                    x: self.pos.0,
                    y: self.pos.1,
                    z: self.pos.2,
    ...
    pub fn spawn_entity(&self, player: &ServerPlayer, world: &World) {
        ...
        match base_kind {
            ...
            BaseKind::Minecart(e::Minecart::Chest { .. }) => {
                self.spawn_entity_object(player, 11, false)
            }
            BaseKind::Minecart(e::Minecart::Furnace { .. }) => {
                self.spawn_entity_object(player, 12, false)
            }
            ...
            BaseKind::Projectile(_, projectile_kind) => match projectile_kind {
            ...
            BaseKind::Living(_, living_kind) => match living_kind {
    ...
    fn spawn_entity_human(
        &self,
        player: &ServerPlayer,
        human: &e::Human,
        metadata: Vec<proto::Metadata>,
    ) {
