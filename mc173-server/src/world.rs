...
use mc173::world::{
    BlockEntityEvent, BlockEntityProgress, BlockEntityStorage, BlockEvent, ChunkEvent, Dimension,
    EntityEvent, Event, Weather, World,
};
...
impl ServerWorld {
    ...
    pub fn tick(&mut self, players: &mut [ServerPlayer]) {
        ...
        self.tick_interval
            .push((start - self.tick_last).as_secs_f32(), 0.02);
        ...
        let mut events = self
            .world
            .swap_events(None)
            .expect("events should be enabled");
        ...
        for event in events.drain(..) {
            match event {
                ...
                Event::Entity { id, inner } => match inner {
                    ...
                    EntityEvent::Remove => self.handle_entity_remove(players, id),
                    EntityEvent::Position { pos } => self.handle_entity_position(id, pos),
                    EntityEvent::Look { look } => self.handle_entity_look(id, look),
                    EntityEvent::Velocity { vel } => self.handle_entity_velocity(id, vel),
                ...
                Event::BlockEntity { pos, inner } => match inner {
                    ...
                    BlockEntityEvent::Remove => self.handle_block_entity_remove(players, pos),
                    ...
                    BlockEntityEvent::Sign => self.handle_block_entity_sign(players, pos),
                },
    ...
    pub fn handle_player_leave(&mut self, player: &mut ServerPlayer, lost: bool) {
        ...
        self.world
            .remove_entity(player.entity_id, "server player leave");
        ...
        if !lost {
            ...
            for entity_id in tracked_entities {
                let tracker = self
                    .entity_trackers
                    .get(&entity_id)
                    .expect("incoherent tracked entity");
    ...
    fn handle_entity_pickup(&mut self, players: &mut [ServerPlayer], id: u32, target_id: u32) {
        let Some(Entity(_, target_kind)) = self.world.get_entity_mut(target_id) else {
            return;
        };
        ...
        let stack = match target_kind {
            ...
            BaseKind::Projectile(projectile, ProjectileKind::Arrow(_)) if projectile.shake == 0 => {
                &mut arrow_stack
            }
    ...
    fn handle_block_entity_storage(
        &mut self,
        players: &mut [ServerPlayer],
        pos: IVec3,
    ...
    fn handle_block_entity_progress(
        &mut self,
        players: &mut [ServerPlayer],
        pos: IVec3,
