...
use crate::proto::{self, InPacket, Network, NetworkClient, OutPacket};
...
impl ServerPlayer {
    ...
    pub fn send_chat(&self, mut message: String) {
        ...
        while slice.len() > 199 {
            ...
            self.send(OutPacket::Chat(proto::ChatPacket {
                message: split.0.to_string(),
            }));
            slice = split.1;
        }
    ...
    pub fn handle(&mut self, world: &mut ServerWorld, packet: InPacket) {
        match packet {
            ...
            InPacket::Disconnect(_) => self.handle_disconnect(),
            InPacket::Chat(packet) => self.handle_chat(world, packet.message),
            InPacket::Position(packet) => self.handle_position(world, packet),
            InPacket::Look(packet) => self.handle_look(world, packet),
            InPacket::PositionLook(packet) => self.handle_position_look(world, packet),
            InPacket::BreakBlock(packet) => self.handle_break_block(world, packet),
            InPacket::PlaceBlock(packet) => self.handle_place_block(world, packet),
            InPacket::HandSlot(packet) => self.handle_hand_slot(world, packet.slot),
            InPacket::WindowClick(packet) => self.handle_window_click(world, packet),
            InPacket::WindowClose(packet) => self.handle_window_close(world, packet),
            InPacket::Animation(packet) => self.handle_animation(world, packet),
            InPacket::Interact(packet) => self.handle_interact(world, packet),
            InPacket::Action(packet) => self.handle_action(world, packet),
            InPacket::UpdateSign(packet) => self.handle_update_sign(world, packet),
    ...
    fn handle_position_look_inner(
        ...
        let entity = sw
            ...
            .expect("incoherent player entity");
        ...
        if pos.is_some() {
            sw.world.push_event(Event::Entity {
                id: self.entity_id,
                inner: EntityEvent::Position { pos: self.pos },
            });
        ...
        if look.is_some() {
            sw.world.push_event(Event::Entity {
                id: self.entity_id,
                inner: EntityEvent::Look { look: self.look },
            });
        }
    }
    ...
    fn handle_break_block(&mut self, sw: &mut ServerWorld, packet: proto::BreakBlockPacket) {
        ...
        let Some(entity) = sw.world.get_entity_mut(self.entity_id) else {
            return;
        };
        ...
        if packet.status == 0 {
            ...
            if let Some((id, _)) = sw.world.get_block(pos) {
                let break_duration = if self.instant_break {
                    ...
                    sw.world
                        .get_break_duration(stack.id, id, in_water, on_ground)
                };
            ...
            if let Some(face) = face {
                match sw.world.interact_block(pos, false) {
                    Interaction::None => {
                        ...
                        sw.world
                            .use_stack(&mut inv, inv_index, pos, face, self.entity_id);
                    }
    ...
    fn handle_window_click(&mut self, sw: &mut ServerWorld, packet: proto::WindowClickPacket) {
        ...
        if self.window.id != packet.window_id {
            warn!(
                "from {}, incoherent window id, expected {}, got {} from client",
                self.username, self.window.id, packet.window_id
            );
            return;
        }
        ...
        if packet.slot == -999 {
            ...
            match self.window.kind {
                WindowKind::Player => {
                    if let Some(main_index) = main_index {
                        ...
                        main_inv
                            .push_front_in(&mut stack, if main_index < 9 { 9..36 } else { 0..9 });
                ...
                WindowKind::Chest { ref pos } => {
                    if let Some(main_index) = main_index {
                        ...
                        // We try to insert
                        for &pos in pos {
                            if let Some(BlockEntity::Chest(chest)) =
                                sw.world.get_block_entity_mut(pos)
                            {
                        ...
                        let pos = pos[slot / 27];
                        let Some(BlockEntity::Chest(chest)) = sw.world.get_block_entity_mut(pos)
                        else {
                            return;
                        };
                ...
                WindowKind::Furnace { pos } => {
                    if let Some(main_index) = main_index {
                        ...
                        let Some(BlockEntity::Furnace(furnace)) =
                            sw.world.get_block_entity_mut(pos)
                        else {
                            return;
                        };
                ...
                WindowKind::Dispenser { pos } => {
                    ...
                    if let Some(main_index) = main_index {
                        slot_stack = main_inv.get(main_index);
                    } else if let Some(BlockEntity::Dispenser(dispenser)) =
                        sw.world.get_block_entity_mut(pos)
                    {
            ...
            let Some(mut slot_handle) = slot_handle else {
                warn!(
                    "from {}, cannot find a handle for slot {} in window {}",
                    self.username, packet.slot, packet.window_id
                );
                return;
            };
            ...
            if slot_stack.is_empty() {
                if !cursor_stack.is_empty() && slot_access.can_drop(cursor_stack) {
                    let drop_size = if packet.right_click {
                        1
                    } else {
                        cursor_stack.size
                    };
                ...
                if (slot_stack.id, slot_stack.damage) != (cursor_stack.id, cursor_stack.damage) {
                    ...
                    let max_stack_size =
                        cursor_item.max_stack_size.min(slot_handle.max_stack_size());
                ...
                if (slot_stack.id, slot_stack.damage) == (cursor_stack.id, cursor_stack.damage) {
                    ...
                    if cursor_stack.size < cursor_item.max_stack_size {
                        ...
                        if available_size >= min_size {
                            ...
                            slot_handle
                                .set_stack(new_slot_stack.to_non_empty().unwrap_or_default());
                        }
                    }
                }
            }
        ...
        match slot_notify {
            ...
            SlotNotify::BlockEntityStorageEvent {
                ...
                sw.world.push_event(Event::BlockEntity {
                    pos,
                    inner: BlockEntityEvent::Storage { storage, stack },
                });
            }
            _ => {}
        }
        ...
        self.send(OutPacket::WindowTransaction(
            proto::WindowTransactionPacket {
                window_id: packet.window_id,
        ...
        if !accepted {
            warn!(
                "from {}, incoherent item at {} in window {}",
                self.username, packet.slot, packet.window_id
            );
        }
    ...
    fn handle_interact(&mut self, sw: &mut ServerWorld, packet: proto::InteractPacket) {
        if self.entity_id != packet.player_entity_id {
            warn!(
                "from {}, incoherent interact entity: {}, expected: {}",
                self.username, packet.player_entity_id, self.entity_id
            );
        }
        ...
        if self.pos.distance_squared(target_base.pos) >= 36.0 {
            warn!(
                "from {}, incoherent interact entity distance",
                self.username
            );
            return;
        }
    ...
    fn handle_action(&mut self, sw: &mut ServerWorld, packet: proto::ActionPacket) {
        if self.entity_id != packet.entity_id {
            warn!(
                "from {}, incoherent player entity: {}, expected: {}",
                self.username, packet.entity_id, self.entity_id
            );
        }
    ...
    fn handle_update_sign(&mut self, sw: &mut ServerWorld, packet: proto::UpdateSignPacket) {
        ...
        let Some(BlockEntity::Sign(sign)) = sw.world.get_block_entity_mut(pos) else {
            warn!(
                "from {}, incoherent update sign, block entity not found at: {pos}",
                self.username
            );
            return;
        };
        ...
        match kind {
            ...
            WindowKind::Furnace { pos } => {
                ...
                if let Some(BlockEntity::Furnace(furnace)) = sw.world.get_block_entity(pos) {
                    self.send(OutPacket::WindowProgressBar(
                        proto::WindowProgressBarPacket {
                            window_id,
                            bar_id: 0,
    ...
    /// on the given slot.
    fn make_window_slot_handle<'a>(
        &'a mut self,
        sw: &'a mut ServerWorld,
        slot: i16,
    ) -> Option<SlotHandle<'a>> {
        ...
        Some(match self.window.kind {
            ...
            WindowKind::CraftingTable { .. } => match slot {
                0 => SlotHandle {
                    kind: SlotKind::CraftingResult {
                        craft_inv: &mut self.craft_inv,
                        craft_tracker: &mut self.craft_tracker,
            ...
            WindowKind::Furnace { pos } => {
                if slot <= 2 {
                    let Some(BlockEntity::Furnace(furnace)) = sw.world.get_block_entity_mut(pos)
                    else {
                        return None;
                    };
                    ...
                    let (stack, access, storage) = match slot {
                        0 => (
                            &mut furnace.input_stack,
                            SlotAccess::PickupDrop,
                            BlockEntityStorage::FurnaceInput,
                        ),
                        1 => (
                            &mut furnace.fuel_stack,
                            SlotAccess::PickupDrop,
                            BlockEntityStorage::FurnaceFuel,
                        ),
                        2 => (
                            &mut furnace.output_stack,
                            SlotAccess::Pickup(1),
            ...
            WindowKind::Dispenser { pos } => {
                if slot < 9 {
                    let Some(BlockEntity::Dispenser(dispenser)) =
                        sw.world.get_block_entity_mut(pos)
                    else {
                        return None;
                    };
    ...
    pub fn drop_stack(&mut self, sw: &mut ServerWorld, stack: ItemStack, on_ground: bool) {
        ...
        let entity = e::Item::new_with(|base, item| {
            ...
            if on_ground {
                ...
                base.vel.x =
                    (-origin_base.look.x.sin() * origin_base.look.y.cos() * drop_speed) as f64;
                base.vel.z =
                    (origin_base.look.x.cos() * origin_base.look.y.cos() * drop_speed) as f64;
            ...
            for cz in (ocz - view_range)..(ocz + view_range) {
                if let Some(chunk) = sw.world.get_chunk(cx, cz) {
                    if self.tracked_chunks.insert((cx, cz)) {
                        ...
                        self.send(OutPacket::ChunkData(new_chunk_data_packet(
                            chunk, from, size,
                        )));
                    }
                }
    ...
    pub fn close_block_window(&mut self, sw: &mut ServerWorld, target_pos: IVec3) {
        let contains = match self.window.kind {
            ...
            WindowKind::Furnace { pos }
            | WindowKind::Dispenser { pos }
            | WindowKind::CraftingTable { pos } => pos == target_pos,
    ...
    pub fn update_block_window_storage(
        &mut self,
        target_pos: IVec3,
    ...
    /// If this player has a window opened for the given position, this will update the
    /// displayed storage according to the given storage event.
    pub fn update_block_window_progress(
        &mut self,
        target_pos: IVec3,
...
impl SlotAccess {
    fn can_drop(self, stack: ItemStack) -> bool {
        match self {
            ...
            SlotAccess::ArmorHelmet => {
                matches!(
                    ...
                    item::LEATHER_HELMET
                        ...
                        | item::IRON_HELMET
                        | item::DIAMOND_HELMET
                ) || stack.id == block::PUMPKIN as u16
            }
            SlotAccess::ArmorChestplate => matches!(
                stack.id,
            ...
            SlotAccess::ArmorLeggings => matches!(
                stack.id,
