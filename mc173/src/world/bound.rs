...
impl World {
    ...
    pub fn get_block_overlay_box(&self, pos: IVec3, id: u8, metadata: u8) -> Option<BoundingBox> {
        let bb = match id {
            ...
            block::RED_MUSHROOM | block::BROWN_MUSHROOM => Face::NegY.extrude(0.3, 0.4),
            block::DEAD_BUSH | block::SAPLING | block::TALL_GRASS => Face::NegY.extrude(0.1, 0.8),
    ...
    pub fn iter_blocks_boxes_in(
        ...
        self.iter_blocks_in(min, max)
            .flat_map(|(pos, id, metadata)| self.iter_block_colliding_boxes(pos, id, metadata))
    }
    ...
    pub fn ray_trace_blocks(
        ...
        while let Some((block, metadata)) = self.get_block(block_pos) {
            let bb = match kind {
                RayTraceKind::Colliding => self.get_block_colliding_box(block_pos, block, metadata),
...
impl Iterator for BlockCollidingBoxesIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let bb = match (index, self.id) {
            (0, block::CACTUS) => {
                BoundingBox::new(PIXEL, 0.0, PIXEL, 1.0 - PIXEL, 1.0, 1.0 - PIXEL)
            }
            (0, block::CAKE) => BoundingBox::new(
                (1 + metadata * 2) as f64 / 16.0,
                0.0,
                PIXEL,
                1.0 - PIXEL,
            ...
            (0, block::LADDER) => {
                return block::ladder::get_face(metadata).map(|face| face.extrude(0.0, PIXEL_2))
            }
            (0, block::SNOW) => {
                let layers = metadata & 7;
                if layers >= 3 {
