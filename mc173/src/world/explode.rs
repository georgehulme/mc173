    ...
    pub fn explode(&mut self, center: DVec3, radius: f32, set_fire: bool, origin_id: Option<u32>) {
                ...
                for dz in 0..16 {
                    if dx == 0 || dx == 15 || dy == 0 || dy == 15 || dz == 0 || dz == 15 {
                        ...
                        while intensity > 0.0 {
                            ...
                            intensity -=
                                (block::material::get_explosion_resistance(block) + 0.3) * STEP;
                            if intensity > 0.0 {
                                if set_fire
                                    && block == block::AIR
        ...
        for (collided_id, Entity(collided_base, _)) in self.iter_entities_colliding(bb) {
            ...
            if dist_norm <= 1.0 {
                ...
                while ray_offset.x <= 1.0 {
                    ...
                    while ray_offset.y <= 1.0 {
                        ...
                        while ray_offset.z <= 1.0 {
                            ray_pass += self
                                .ray_trace_blocks(center, ray + ray_offset, RayTraceKind::Overlay)
                                .is_none() as usize;
                            ray_count += 1;
                            ray_offset.z += step.z;
                        }
                ...
                let damage =
                    (damage_factor * damage_factor + damage_factor) / 2.0 * 8.0 * radius + 1.0;
