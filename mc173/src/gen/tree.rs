...
impl FeatureGenerator for BigTreeGenerator {
    fn generate(&mut self, world: &mut World, pos: IVec3, rand: &mut JavaRandom) -> bool {
        ...
        if !matches!(
            world.get_block(pos - IVec3::Y),
            Some((block::GRASS | block::DIRT, _))
        ) {
            return false;
        }
        ...
        let nodes_per_height =
            ((1.382 + (self.leaf_density * height as f32 / 13.0).powi(2)) as i32).max(1) as usize;
...
impl BigTreeGenerator {
    ...
    fn place_big_tree_leaf_layer(&self, world: &mut World, pos: IVec3, radius: f32) {
        ...
        for dx in -block_radius..=block_radius {
            for dz in -block_radius..=block_radius {
                let dist =
                    ((dx.abs() as f32 + 0.5).powi(2) + (dz.abs() as f32 + 0.5).powi(2)).sqrt();
                if dist <= radius {
    ...
    fn check_big_tree_branch(&self, world: &mut World, from: IVec3, to: IVec3) -> Option<IVec3> {
        BlockLineIter::new(from, to)
            .find(|&pos| !matches!(world.get_block(pos), Some((block::AIR | block::LEAVES, _))))
    }
...
impl Iterator for BlockLineIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.major == self.major_max {
            ...
            pos[self.second_axis] =
                (self.from[self.second_axis] as f32 + self.major as f32 * self.second_ratio + 0.5)
                    .floor() as i32;
            pos[self.third_axis] =
                (self.from[self.third_axis] as f32 + self.major as f32 * self.third_ratio + 0.5)
                    .floor() as i32;
