...
use super::{BaseKind, Entity, EntityCategory, LivingKind};
...
fn tick_living_ai(world: &mut World, _id: u32, entity: &mut Entity) {
    ...
    if base.rand.next_float() < 0.02 {
        if let Some((target_entity_id, _, _)) =
            common::find_closest_player_entity(world, base.pos, LOOK_AT_MAX_DIST)
        {
...
fn tick_ground_ai(world: &mut World, id: u32, entity: &mut Entity) {
    ...
    if let Some(target_id) = living.attack_target {
        ...
        if search_around {
            if let Some((target_id, Entity(target_base, _), _)) =
                common::find_closest_player_entity(world, base.pos, 16.0)
            {
    ...
    let attack_target = living.attack_target.map(|id| world.get_entity(id).unwrap());
        ...
        if base.rand.next_int_bounded(100) != 0 {
            ...
            if let Some(next_pos) = next_pos {
                ...
                let move_speed = match living_kind {
                    LivingKind::Giant(_) | LivingKind::Zombie(_) | LivingKind::PigZombie(_) => 0.5,
...
fn tick_ghast_ai(world: &mut World, id: u32, entity: &mut Entity) {
    ...
    if target_entity.is_none() || ghast.attack_target_time == 0 {
        if let Some((closest_id, closest_entity, _)) =
            common::find_closest_player_entity(world, base.pos, 100.0)
        {
        ...
        if target_base.pos.distance_squared(base.pos) < SHOT_MAX_DIST_SQUARED {
            ...
            if common::can_eye_track(world, base, target_base) {
                ...
                if living.attack_time == 60 {
                    ...
                    let fireball =
                        Fireball::new_with(|throw_base, throw_projectile, throw_fireball| {
