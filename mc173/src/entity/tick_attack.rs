...
use super::{BaseKind, Entity, LivingKind};
...
pub(super) fn tick_attack(
    ...
    match entity {
        Entity(_, BaseKind::Living(_, LivingKind::Spider(_))) => tick_spider_attack(
            world,
            id,
            entity,
            target_id,
        ...
        Entity(_, BaseKind::Living(_, LivingKind::Creeper(_))) => tick_creeper_attack(
            world,
            id,
            entity,
            target_id,
        ...
        Entity(_, BaseKind::Living(_, LivingKind::Skeleton(_))) => tick_skeleton_attack(
            world,
            id,
            entity,
            target_id,
        ...
        Entity(_, BaseKind::Living(_, _)) => tick_mob_attack(
            world,
            id,
            entity,
            target_id,
...
fn tick_mob_attack(
    ...
    if eye_track && living.attack_time == 0 && dist_squared < MAX_DIST_SQUARED {
        let Some(Entity(target_base, BaseKind::Living(_, _))) = world.get_entity_mut(target_id)
        else {
