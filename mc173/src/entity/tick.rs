...
pub(super) fn tick(world: &mut World, id: u32, entity: &mut Entity) {
    ...
    if prev_pos != base.pos {
        world.push_event(Event::Entity {
            id,
            inner: EntityEvent::Position { pos: base.pos },
        });
    }
    ...
    if prev_vel != base.vel {
        world.push_event(Event::Entity {
            id,
            inner: EntityEvent::Velocity { vel: base.vel },
        });
    }
    ...
    if prev_look != base.look {
        world.push_event(Event::Entity {
            id,
            inner: EntityEvent::Look { look: base.look },
        });
    }
}
...
fn tick_projectile(world: &mut World, id: u32, entity: &mut Entity) {
    ...
    let hit_entity = world
        .iter_entities_colliding_mut(base.bb.offset(base.vel).inflate(DVec3::ONE))
        // Filter out entities that we cannot collide with.
        .filter(|(target_id, Entity(_, target_base_kind))| {
            match target_base_kind {
                BaseKind::Item(_) | BaseKind::LightningBolt(_) | BaseKind::Projectile(_, _) => {
                    false
                }
    ...
    if let ProjectileKind::Fireball(fireball) = projectile_kind {
        ...
        if bobber.catch_time > 0 {
            base.vel.y -= (base.rand.next_float() * base.rand.next_float() * base.rand.next_float())
                as f64
                * 0.2;
        }
...
fn tick_living_push(world: &mut World, _id: u32, base: &mut Base) {
    ...
    for (_, push_entity) in
        world.iter_entities_colliding_mut(base.bb.inflate(DVec3::new(0.2, 0.0, 0.2)))
    {
        let Entity(push_base, push_base_kind) = push_entity;

        match push_base_kind {
            BaseKind::Boat(_) | BaseKind::Living(_, _) | BaseKind::Minecart(_) => {}
...
pub fn apply_base_vel(
    ...
    if base.no_clip {
        ...
        common::BOUNDING_BOX.with_borrow_mut(|colliding_bbs| {
            ...
            colliding_bbs.extend(world.iter_entities_colliding(colliding_bb).filter_map(
                |(_entity_id, entity)| {
