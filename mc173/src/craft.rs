...
macro_rules! tool {
    ( pickaxe $result:expr, $mat:expr ) => {
        Recipe::new_shaped(
            ItemStack::new_single($result, 0),
            &[$mat, $mat, $mat, EMPTY, STICK, EMPTY, EMPTY, STICK, EMPTY],
            3,
        )
    };
    ...
    ( hoe $result:expr, $mat:expr ) => {
        Recipe::new_shaped(
            ItemStack::new_single($result, 0),
            &[$mat, $mat, STICK, EMPTY, STICK, EMPTY],
            2,
        )
    };
...
macro_rules! armor {
    ...
    ( chestplate $result:expr, $mat:expr ) => {
        Recipe::new_shaped(
            ItemStack::new_single($result, 0),
            &[$mat, EMPTY, $mat, $mat, $mat, $mat, $mat, $mat, $mat],
            3,
        )
    };
    ( leggings $result:expr, $mat:expr ) => {
        Recipe::new_shaped(
            ItemStack::new_single($result, 0),
            &[$mat, $mat, $mat, $mat, EMPTY, $mat, $mat, EMPTY, $mat],
            3,
        )
    };
...
const RECIPES: &[Recipe] = &[
    ...
    Recipe::new_shaped(
        RAIL_16,
        &[
            IRON_INGOT, EMPTY, IRON_INGOT, IRON_INGOT, STICK, IRON_INGOT, IRON_INGOT, EMPTY,
            IRON_INGOT,
        ],
        3,
    ),
    Recipe::new_shaped(
        POWERED_RAIL_6,
        &[
            GOLD_INGOT, EMPTY, GOLD_INGOT, GOLD_INGOT, STICK, GOLD_INGOT, GOLD_INGOT, REDSTONE,
            GOLD_INGOT,
        ],
        3,
    ),
    ...
    Recipe::new_shaped(
        GOLD_APPLE,
        &[
            GOLD_INGOT, GOLD_INGOT, GOLD_INGOT, GOLD_INGOT, APPLE, GOLD_INGOT, GOLD_INGOT,
            GOLD_INGOT,
        ],
        3,
    ),
    ...
    Recipe::new_shaped(
        CLOCK,
        &[
            EMPTY, GOLD_INGOT, EMPTY, GOLD_INGOT, REDSTONE, GOLD_INGOT, EMPTY, GOLD_INGOT, EMPTY,
        ],
        3,
    ),
    Recipe::new_shaped(
        COMPASS,
        &[
            EMPTY, IRON_INGOT, EMPTY, IRON_INGOT, REDSTONE, IRON_INGOT, EMPTY, IRON_INGOT, EMPTY,
        ],
        3,
    ),
    Recipe::new_shaped(
            ...
            'out: for start_y in 0..=(3 - recipe_height) {
                ...
                for dx in 0..3 {
                    for dy in 0..3 {
                        ...
                        if dx < start_x
                            || dx >= start_x + recipe_width
                            || dy < start_y
                            || dy >= start_y + recipe_height
                        {
                            ...
                            if normal_valid {
                                let normal_stack =
                                    self.pattern[pattern_x + pattern_y * recipe_width];
                            ...
                            if flip_valid {
                                let flip_stack =
                                    self.pattern[flip_pattern_x + pattern_y * recipe_width];
            ...
            if !stack.is_empty() {
                for (i, pat_stack) in self.pattern.iter().copied().enumerate() {
                    if pat_matched & (1 << i) == 0
                        && (pat_stack.id, pat_stack.damage) == (stack.id, stack.damage)
                    {
                        pat_matched |= 1 << i;
                        continue 'inv;
                    }
